...
#numBWrap.noDec .zero { grid-column: span 3; }
  ...
  <body>
    <!-- (B) INPUT FIELDS -->
    <!-- Field A: <input type="text" id="CoinsToBeUsed"/>
    <br><br>
    Field B: <textarea id="DiscountOverall"></textarea> -->

    <!-- (C) ATTACH NUMPAD -->
    <script>
    window.addEventListener("load", function(){
      // (C1) BASIC
      numpad.attach({target: document.getElementById("CoinsToBeUsed"),
        onselect : function () { 
          UseCoins();
        }
      });
      ...
      // (C2) ALL OPTIONS
      numpad.attach({
        target: document.getElementById("DiscountOverall"),
        onselect : function () {
          calc();
        }
      });
      ...
      // (C2) ALL OPTIONS
      numpad.attach({target: document.getElementById("tax"),
        onselect : function () { 
          calc();
        }
      });
